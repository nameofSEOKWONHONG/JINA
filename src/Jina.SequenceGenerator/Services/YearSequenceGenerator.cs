...
public class YearSequenceGenerator : ISequenceGenerator
{
    private readonly DbContext _db;
    public YearSequenceGenerator(DbContext db)
    {
        _db = db;
    }
    ...
    public async Task<Int64> CreateAsync(SequenceOption option)
    {
        ...
        var seq = _db.Set<YearSequence>();
        var max = await seq.Where(m => m.TenantId == option.TenantId && 
                                       m.TableName == option.TableName &&
                                       m.Year == option.Year).MaxAsync(m => m.Id);
        if (max <= 0)
        {
            max = option.Length;
            if (max <= 0) max = 1;
            await seq.AddAsync(new(option.TenantId, option.TableName, option.Year, max));
        }
        else
        {
            if (option.Length > 0)
            ...
            seq.Update(entity: new(option.TenantId, option.TableName, option.Year, max));
        }
